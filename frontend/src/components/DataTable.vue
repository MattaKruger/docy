    ...
    items = items.filter((item) => {
      ...
      const searchInObject = (obj: any): boolean => {
        ...
        return Object.keys(obj).some((key) => {
          ...
          // Check if primitive value contains the search query
...
<template>
  <div class="rounded-lg shadow overflow-hidden" :class="bgColor">
    <!-- Table Toolbar -->
    <div class="p-4 border-b border-gray-200 flex flex-wrap justify-between items-center">
      <div v-if="searchable" class="w-full md:w-auto mb-4 md:mb-0">
        <div class="relative">
          <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-white"> 🔍 </span>
          <input
            type="text"
            v-model="searchQuery"
            @input="resetPagination"
            placeholder="Search..."
            class="pl-10 pr-4 py-2 border border-gray-300 rounded-md w-full md:w-64 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          />
        </div>
      </div>
    ...
    <div class="overflow-x-auto">
      <table class="table min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200" :class="{ 'animate-pulse': loading }">
          <template v-if="loading">
            <tr v-for="i in 3" :key="`skeleton-${i}`">
              <td v-if="selectable" class="py-4 px-4">
                <div class="h-4 w-4 bg-gray-200 rounded"></div>
              </td>
              <td v-for="column in columns" :key="`skeleton-${i}-${column.key}`" class="py-4 px-4">
                <div class="h-4 bg-gray-200 rounded w-3/4"></div>
              </td>
              <td v-if="actions.length > 0" class="py-4 px-4">
