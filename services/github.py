...
class GithubService:
    ...
    def __init__(self):
        ...
        self.chroma_db_client = ChromaService(embedding_model="all-mpnet-base-v2")
    ...
    def _process_content_file(
        self, content_file: ContentFile, docs_content: Dict[str, str]
    ) -> None:
    ...
    def _extract_docs_from_path(
        self, repo: Repository, docs_path: str
    ) -> Dict[str, str]:
            ...
            if isinstance(contents, list):
                for content_file in contents:
                    if content_file.type == "file":
                        ...
                        docs_content.update(
                            self._extract_docs_from_path(repo, content_file.path)
                        )
            elif isinstance(contents, ContentFile):
                self._process_content_file(contents, docs_content)
            else:
                logger.info(
                    f"Warning: Unexpected content type at {docs_path} in {repo.full_name}"
                )
    ...
    def extract_repo_docs(
        self, repo_name: str, docs_path: Optional[str] = "docs"
    ) -> RepoDocs:
        """
        ...
        query_results = self.chroma_db_client.get_documents(
            where_filter={
                "$and": [
                    {"repo_name": repo_name},
        ...
        if docs_content or readme_content:
            ...
            documents = (
                list(docs_content.values())
                if docs_content
                else [readme_content] if readme_content else []
            )
            if docs_path:
                metadata_docs = [
                    {"repo_name": repo_name, "doc_path": path, "docs_path": docs_path}
                    for path in ids
                ]
            else:
                metadata_docs = (
            ...
            if documents:
                self.chroma_db_client.add_documents(
                    collection="test",
        ...
        extracted_data = RepoDocs(
            readme_content=readme_content, docs_content=docs_content, error=None
        )
