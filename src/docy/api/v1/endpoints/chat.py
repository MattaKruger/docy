...
from docy.models import Chat, Message
from docy.repositories import ChatRepository, MessageRepository
from docy.schemas.chat import ChatIn, ChatOut, ChatUpdate, MessageIn, MessageOut, MessageOut, MessageUpdate
...
async def add_agent_to_chat(
    chat_id: int = Path(...), agent_id: int = Path(...), chat_repo: ChatRepository = Depends(get_chat_repo)
):
    pass


@router.post("/{chat_id}/message", response_model=int)
